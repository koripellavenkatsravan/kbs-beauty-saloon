...
import { ArrowLeft, Award, LogOut, ShoppingBag, Calendar } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#FDFBF7]">
      ...
      <div className="max-w-5xl mx-auto px-5 sm:px-10 py-10">
        ...
        <div className="mt-4 grid md:grid-cols-3 gap-4">
          <motion.div initial={{opacity:0,y:6}} animate={{opacity:1,y:0}} className="kbs-card rounded-3xl p-6 md:col-span-2">
            <div className="text-[10px] uppercase tracking-[0.24em] text-[#8a6c1e]">Signed In</div>
            <div className="font-serif-kbs text-3xl text-[#1A1A1A] mt-1">{user.name}</div>
            <div className="text-sm text-[#6b6b6b]">{user.email}{user.phone ? ` · ${user.phone}` : ""}</div>
          ...
          <motion.div initial={{opacity:0,y:6}} animate={{opacity:1,y:0}} className="kbs-card rounded-3xl p-6 bg-gradient-to-br from-[#FBF3E6] to-white">
            <div className="text-[10px] uppercase tracking-[0.24em] text-[#8a6c1e] inline-flex items-center gap-1"><Award size={12}/> Loyalty</div>
            <div className="font-serif-kbs text-5xl text-[#B7902B] mt-1">{user.loyalty_points || 0}</div>
            <div className="text-xs text-[#6b6b6b]">points · 100 pts = ₹100 off</div>
          </motion.div>
        </div>
        ...
        <div className="mt-8 grid lg:grid-cols-2 gap-6">
          <div className="kbs-card rounded-3xl p-6">
            ...
            {bookings.length === 0 && <div className="text-sm text-[#6b6b6b] mt-4">No bookings yet.</div>}
            <div className="mt-4 space-y-2 max-h-96 overflow-y-auto menu-scroll">
              {bookings.map((b) => (
                <div key={b.id} className="border border-[#E7DFCF] rounded-xl p-3 bg-white">
                  ...
                  <div className="text-xs text-[#6b6b6b]">Stylist: {b.stylist} · {b.services.length} service{b.services.length!==1?"s":""} · ₹{b.total}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="kbs-card rounded-3xl p-6">
            ...
            {orders.length === 0 && <div className="text-sm text-[#6b6b6b] mt-4">No orders yet.</div>}
            <div className="mt-4 space-y-2 max-h-96 overflow-y-auto menu-scroll">
              {orders.map((o) => (
                <div key={o.id} className="border border-[#E7DFCF] rounded-xl p-3 bg-white">
                  <div className="flex items-center justify-between">
                  ...
                  <div className="text-xs text-[#6b6b6b]">{o.items.length} item{o.items.length!==1?"s":""} · Total ₹{o.total}</div>
                </div>
              ))}
            </div>
          </div>
